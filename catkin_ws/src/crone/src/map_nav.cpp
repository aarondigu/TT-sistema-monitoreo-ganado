...
int main(int argc, char** argv){
  ...
  while (node.ok()){
    ...
    float thresh = 4.0; // Limite de distancia para considerar como ocupado

    for (int i = 0; i < size_x; i++) 
    { 
       for (int j = 0; j < size_y; j++) 
       {
        if (histo_dis[i][j] != 0.0 && histo_dis[i][j] <= thresh && histo_prim[i][j] > 10) // Se considera la distancia y cuantos puntos hay en cada celda
        {
          histo_bin[i][j] = true;
        }
        else
    ...
    float r_alarg = 0.80; // Radio de alargamiento en metros
    ...
    float dis_ref = 2.5; // Distancia de referencia para colocar el siguiente punto en metros
    ...
    // Distancia al waypoint goal actual
    float dis_wp = sqrt(pow(wp_goal_base.point.x,2) + pow(wp_goal_base.point.y,2) + pow(wp_goal_base.point.z,2));
    if (dis_wp < dis_ref) // Ajusta la distancia de referencia para que no se pase el waypoint
      dis_ref = dis_wp;
    ...
    // Casos en que el angulo de azimuth no recaiga en el histograma 
    if (ang_az < 5*res) // Factor de seguridad
      ang_az = 5*res;
    else if (ang_az > fov_x - 5*res)
      ang_az = fov_x - 5*res;
    // Casos en que el angulo de aelevacion no recaiga en el histogram
    if (ang_el < 3*res)
      ang_el = 3*res;
    else if (ang_el > fov_y - 3*res)
      ang_el = fov_y - 3*res;

    // Obtener en que celda del histograma recae
    index_x = (int)ang_az/res;
    index_y = (int)ang_el/res;
       ...
       { 
          ...
          if ( histo_bin_a[i][j] == 0){ // Si la celda esta libre
            ...
            if (cost_total <= cost_total_min){ // Si su costo es el minimo, se vuelve la celda ganadora, a menos de que otra sea menor y la reemplace
              ...
              x_min = i; // Guarda el indice de la celda ganadora en x
              y_min = j; // Guarda el indice de la celda ganadora en y
            }
    ...
    // Imprime hisotgrama binario alargado en colores
    string color = RESET;
    for (int j = size_y - 1; j >= 0; j--) 
    { 
       for (int i = size_x -1; i >= 0; i--) 
   ...
   if (histo_vacio == size_x * size_y){ // Si el histograma binario alargado se encuentra completamente ocupado

      escala_vel = 0.0; 

      win_x = 14;
      win_y = 9; //Reinicia los ganadores

      x_ref = -20.0; // Se regresa un poco
      y_ref = 0.0;
      z_ref = 0.0;

      ang_az = 180.0/PI * atan2(wp_goal_base.point.y, wp_goal_base.point.x) + fov_x/2; // Obtiene el yaw hacia el siguiente waypoint goal
      if (ang_az >= 0.0)
        ang_az = 10.0 * PI/180.0; // Gira 10 grados en direccion del waypoint goal
      else 
        ang_az = -10.0 * PI/180.0; // Gira 10 grados en direccion del waypoint goal

      cout << "Histograma lleno de obstaculos" << endl;
    }
    else { // Si el histograma binario alargado NO se encuentra completamente ocupado
